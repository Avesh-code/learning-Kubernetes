...
```
## 3. Init Containers:
* Init containers in Kubernetes are specialized containers that run before the main application containers in a pod. Their primary role is to perform setup or initialization tasks—such as preparing configuration files, waiting for external services to be ready, or running database migrations—ensuring all prerequisites are met before the main containers start. Init containers execute sequentially, each must complete successfully, and they enable a clean, modular startup process separate from the main application logic.
...
* Sidecar containers in Kubernetes are additional containers that run alongside the main application container within the same pod. They extend or enhance the functionality of the main container by handling auxiliary tasks such as logging, monitoring, security, or data synchronization without modifying the main application. Sidecars share the same network and storage as the primary container, allowing close integration while keeping responsibilities separate for modularity and easier maintenance.
...
kubectl get pods
kubectl logs side-car-test -c main-container
kubectl logs side-car-test -c sidecar-container
